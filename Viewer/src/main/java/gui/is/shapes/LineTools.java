...
public class LineTools {
	...
	public static LineCoordinates findExternalPoints(ArrayList<Coordinate> crosses) {
		Coordinate meanCoord = new CoordBounds(crosses).getMiddle();
		Coordinate start = PointTools.findFarthestPoint(crosses, meanCoord);
		Coordinate end = PointTools.findFarthestPoint(crosses, start);
	 ...
	 * @throws Exception
	 */
	public static Coordinate FindLineIntersection(LineCoordinates first, LineCoordinates second)
			throws Exception {
	 ...
	 */
	public static LineCoordinates findClosestLineToPoint(Coordinate point, List<LineCoordinates> list) {
		LineCoordinates answer = list.get(0);
		double shortest = Double.MAX_VALUE;

		for (LineCoordinates line : list) {
			...
			Coordinate shorterpnt = ans1 < ans2 ? line.getStart() : line.getEnd();
