...
import com.mapviewer.gui.core.mapTileControl.MapTile;
import com.mapviewer.gui.core.mapTileControl.TileController;
import com.mapviewer.gui.core.mapTileSources.BingAerialTileSource;
import com.mapviewer.gui.core.mapTileSources.OsmCartoMapTileSource;
import com.mapviewer.gui.core.mapTileSources.OsmCycleMapTileSource;
import com.mapviewer.gui.core.mapTileSources.OsmDarkMapTileSource;
import com.mapviewer.gui.core.mapTileSources.OsmHotMapTileSource;
import com.mapviewer.gui.core.mapTileSources.OsmIntelMapTileSource;
import com.mapviewer.gui.core.mapViewer.ViewMap;
...
import com.mapviewer.gui.is.interfaces.maptiles.TileSource;
import com.mapviewer.gui.is.mapTileSources.OsmMercator;
 ...
 * @author smithjel
 */
public abstract class ViewMapImpl extends Pane implements ViewMap {
	...
	private Map<Integer, MapObject> mapObjectMap;
	...
	private ViewMapImpl(TileSource ts, int x, int y, int width, int height, int initial_zoom ) {
		...
		if (bottomPane != null) {
			...
			//bottomPane.setLayoutX( (((MapWidth.get() - Screen.getPrimary().getBounds().getWidth() * AppConfig.FRAME_CONTAINER_REDUCE_PRECENTAGE) / 2) - (bottomPane.getWidth() / 2)) );
			bottomPane.setLayoutY( MapHeight.doubleValue() - LOCATION_LABEL_PADDING );
		}
...
//		setMapBounds( 0, 0, _WIDTH.get(), _HEIGHT.get() );
	...
	public Pane getMapTopPane() {
		...
		btnMapMenu.setOnAction( (event) -> bottomPane.setVisible(!bottomPane.isVisible()));
	...
	protected void initializeZoomSlider() {
		...
		Image imagePlus = new Image(this.getClass().getResource("/com/mapviewer/mapImages/plus.png").toString());
		zoomInButton = new Button();
		zoomInButton.setGraphic(new ImageView(imagePlus));
		zoomInButton.setOnAction( e -> {if( IgnoreRepaint == false ) zoomIn();});

		Image imageMinus = new Image(this.getClass().getResource("/com/mapviewer/mapImages/minus.png").toString());
	...
	public void addMapLine(MapLine line) {
		if (!getMapObjectMap().containsKey(line.hashCode())) {
			getMapObjectMap().put(line.hashCode(), line);
	...
	public void removeMapLine(MapLine line) {
		if (getMapObjectMap().containsKey(line.hashCode())) {
			getMapObjectMap().remove(line.hashCode());
	...
	public void addMapMarker(MapMarker marker) {
		if (!getMapObjectMap().containsKey(marker.hashCode())) {
			getMapObjectMap().put(marker.hashCode(), marker);
	...
	public void removeMapMarker(MapMarker marker) {
		if (getMapObjectMap().containsKey(marker.hashCode())) {
			getMapObjectMap().remove(marker.hashCode());
	...
	public void addMapPolygon(MapPolygon polygon) {
		if (!getMapObjectMap().containsKey(polygon.hashCode())) {
			getMapObjectMap().put(polygon.hashCode(), polygon);
	...
	public void removeMapPolygon(MapPolygon polygon) {
		if (getMapObjectMap().containsKey(polygon.hashCode())) {
			getMapObjectMap().remove(polygon.hashCode());
	...
	public void setShowToolbars(boolean visible) {
		...
		this.bottomPane.setVisible(visible);
	}
